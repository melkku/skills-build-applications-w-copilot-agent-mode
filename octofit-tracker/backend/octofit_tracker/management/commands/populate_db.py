...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        Activity.objects.create(user=users[1], activity_type='pushups', duration=20, date=timezone.now().date())
        Activity.objects.create(user=users[2], activity_type='deadlift', duration=40, date=timezone.now().date())
        Activity.objects.create(user=users[3], activity_type='run', duration=25, date=timezone.now().date())
